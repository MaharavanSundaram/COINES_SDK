...
tar zcf __data.tar.gz -C $RELEASE_DIR/.. __temp
cat ./License_Parsing_Script __data.tar.gz >coines_installer.sh
rm __data.tar.gz
chmod +x coines_installer.sh

# Rename installer with version information
TAG_DESC=$(git describe --tags)
mv coines_installer.sh $TAG_DESC.sh

tar -czvf ${TAG_DESC}_installer_macos.tar.gz $TAG_DESC.sh
